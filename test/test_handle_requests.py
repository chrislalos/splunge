...
class HandleRequestTests(unittest.TestCase):
	...
	def test_markup(self):
		# Create a xgi, then create + execute a handler for it
		(handler, resp) = create_and_test_handler(self, "/meat/bar?name=meat", PythonTemplateHandler)
		# Test the response
		test_response_ok_html(self, resp)
	...
	def test_module_plus_markup(self):
		# Create a xgi, then create + execute a handler for it
		(handler, resp) = create_and_test_handler(self, "/meat/foo3", PythonModuleHandler)
		# Test the response
		test_response_ok_html(self, resp)
	...
	def test_module_with_under(self):
		# Create a xgi, then create + execute a handler for it
		(handler, resp) = create_and_test_handler(self, "/meat/foo2", PythonModuleHandler)
		# Test the response
		test_response_ok_html(self, resp)
	
	def test_python_source(self):
		# Create a xgi, then create + execute a handler for it
		(handler, resp) = create_and_test_handler(self, '/meat/foo.py', SourceHandler)
		# Test the response
		test_response_ok_html(self, resp)
